...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  //if(numAColumns%TILE_WIDTH) 
    //numTile = numAColumns/TILE_WIDTH + 1;
  //else
    //numTile = numAColumns/TILE_WIDTH;
  for( int m = 0; m < ceil(numAColumns/16.0); ++m)
  {
    if((Row < numARows) && ((m * TILE_WIDTH + tx) < numAColumns))
      subTileA[ty][tx] = A[numAColumns * Row + m * TILE_WIDTH + tx];
    else
      subTileA[ty][tx] = 0.0;
    if((Col < numBColumns) && ((m * TILE_WIDTH + ty) < numBRows))
      subTileB[ty][tx] = B[numBColumns * ( m * TILE_WIDTH + ty) + Col];
    else
      subTileB[ty][tx] = 0.0;
    __syncthreads();
